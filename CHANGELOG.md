...
- Polling hints on /changes endpoint -- response now includes polling object with next_since timestamp, suggested_interval_seconds (30s), change_count, and contextual hint message; also sets X-Poll-Interval, X-Server-Time, and X-Change-Count response headers to help agents optimize their polling strategy
